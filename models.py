# RoadFreq-GAN: generator and discriminator architectures
# Modified from FastGAN with CASEBlock and frequency-domain loss
import torch
...
class CASEBlock(nn.Module):
    def __init__(self, ch_in, ch_out, reduction=16):
        ...
        mip = max(8, ch_out // reduction)
        self.ca_conv1 = nn.Conv2d(ch_out, mip, kernel_size=1, stride=1, padding=0)
        self.ca_bn1 = nn.BatchNorm2d(mip)
        self.ca_act = Swish()

        self.ca_conv_h = nn.Conv2d(mip, ch_out, kernel_size=3, stride=1, padding=2, dilation=2)
        self.ca_conv_w = nn.Conv2d(mip, ch_out, kernel_size=3, stride=1, padding=2, dilation=2)

    def forward(self, feat_small, feat_big):
        out = feat_big * self.se_main(feat_small)
        ...
        a_h = torch.sigmoid(self.ca_conv_h(x_h))
        a_w = torch.sigmoid(self.ca_conv_w(x_w))
...
class Generator(nn.Module):
    ...
    def __init__(self, ngf=64, nz=100, nc=3, im_size=1024):
        ...
        self.se_64  = CASEBlock(nfc[4], nfc[64])
        self.se_128 = CASEBlock(nfc[8], nfc[128])
        self.se_256 = CASEBlock(nfc[16], nfc[256])
        ...
        if im_size > 256:
            ...
            self.se_512 = CASEBlock(nfc[32], nfc[512])
        if im_size > 512:
...
class Discriminator(nn.Module):
    ...
    def __init__(self, ndf=64, nc=3, im_size=512):
        ...
        self.se_2_16 = CASEBlock(nfc[512], nfc[64])
        self.se_4_32 = CASEBlock(nfc[256], nfc[32])
        self.se_8_64 = CASEBlock(nfc[128], nfc[16])
